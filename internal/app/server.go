...
const (
	...
	SectionPlaneListToolName   = "section_plane.list"
	SectionPlaneCreateToolName = "section_plane.create"
	SceneListToolName          = "scene.list"
	...
	ServerInstructions = "Start with sketchup.sessions.list and choose one live session. Read model.summary before any write and use the returned model GUID and revision. For LayOut documentation: read model.bounds, list and reuse existing scenes/section planes before creating presentation state, save a .skp copy, inspect the .layout template, create from that template, add viewports/dimensions/annotations with panel_id, validate every populated panel, then export. Prefer named scenes for viewport presentation; orthographic views need an explicit scale denominator; accepted drawings should normally use require_fit=true with model-space fit_model_bounds_mm. Paper-space coordinates are millimeters. Reuse durable entity references, give every intended write a unique operation_id, and on STALE_REVISION refresh state before retrying with a new operation_id."
)
